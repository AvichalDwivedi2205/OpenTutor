  ...
  const startUpload = async () => {
    ...
    for (const urlImport of urlImports.filter(u => u.status === "pending")) {
      ...
      for (let progress = 0; progress <= 100; progress += 20) {
        ...
        setUrlImports(prev => prev.map(u => 
          u.id === urlImport.id ? { ...u, progress } : u
        ));
      }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Add new materials to your collection
              </p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-6xl p-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className={`inline-flex h-10 w-10 items-center justify-center rounded-lg flex-shrink-0 ${type.color}`}>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-xl border border-black/10 bg-white/80 p-6 backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold">
                  {importMethod === "upload" ? "Upload Files" : "Import from URL"}
                </h2>
              ...
              {importMethod === "upload" ? (
                ...
                >
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                    Supported formats: {materialTypes.find(t => t.type === selectedType)?.acceptedFormats}
                  </p>
                  <input
                    type="file"
                    multiple
                  ...
                  <label
                    htmlFor="file-upload"
                    className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 cursor-pointer dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
                  >
                    <Upload className="h-4 w-4" />
                    Choose Files
                  </label>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full rounded-lg border border-black/10 bg-white/50 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/20 dark:border-white/10 dark:bg-slate-800 dark:text-white"
                        />
                      </div>
                      <div>
                ...
                <div className="mt-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-medium">
                      {importMethod === "upload" ? "Selected Files" : "URLs to Import"}
                    </h3>
                    <div className="text-sm text-slate-600 dark:text-slate-400">
                      {importMethod === "upload" ? uploadFiles.length : urlImports.length} items
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                    {/* File uploads */}
                    {importMethod === "upload" && uploadFiles.map((uploadFile) => {
                      ...
                      return (
                        ...
                        >
                          <FileIcon className="h-10 w-10 text-slate-400 flex-shrink-0" />
                          <div className="flex-1 min-w-0">
                    ...
                    {/* URL imports */}
                    {importMethod === "url" && urlImports.map((urlImport) => (
                      <motion.div
                        key={urlImport.id}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="flex items-center gap-3 rounded-lg border border-black/10 bg-white/50 p-4 dark:border-white/10 dark:bg-white/5"
                      >
                        <div className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-400 flex-shrink-0">
                          <Globe className="h-5 w-5" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium truncate">{urlImport.title}</p>
                          <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                  ...
                  {/* Action Button */}
                  {((importMethod === "upload" && uploadFiles.some(f => f.status === "pending")) ||
                    (importMethod === "url" && urlImports.some(u => u.status === "pending"))) && (
            ...
            {/* Success Summary */}
            {(uploadFiles.some(f => f.status === "success") || urlImports.some(u => u.status === "success")) && (
              <div className="rounded-xl border border-green-200 bg-green-50 p-6 dark:border-green-800 dark:bg-green-900/20">
                ...
                <p className="text-sm text-green-700 dark:text-green-300 mb-4">
                  Your materials have been added to your collection and are now available 
                  for use in any workspace. They will enhance AI-generated content and recommendations.
                </p>
                <div className="flex items-center gap-2">
                  <Link
                    href="/materials"
        ...
        <div className="mt-8 rounded-xl border border-black/10 bg-white/80 p-6 backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-4">
            <div className="text-center">
              <div className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-sky-100 text-sky-600 dark:bg-sky-900 dark:text-sky-400 mb-3">
                1
              </div>
              <h3 className="font-medium mb-1">Select Type</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400">
            ...
            <div className="text-center">
              ...
              <h3 className="font-medium mb-1">Choose Method</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400">
            ...
            <div className="text-center">
              ...
              <h3 className="font-medium mb-1">Add Materials</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400">
            ...
            <div className="text-center">
              ...
              <h3 className="font-medium mb-1">Global Access</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Materials are available across all workspaces
              </p>
            </div>
          </div>
        </div>
